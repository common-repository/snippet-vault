...
class Meow_MWCODE_Rest
{
	...
	private $namespace = 'snippet-vault/v1';
	...
	function rest_settings_update( $request ) {
		try {
			...
			$success = !!$options;
			$message = __( $success ? 'OK' : "Could not update options.", 'snippet-vault' );
	...
	function rest_settings_reset() {
		try {
			...
			$success = !!$options;
			$message = __( $success ? 'OK' : "Could not reset options.", 'snippet-vault' );
			return new WP_REST_Response([ 'success' => $success, 'message' => $message, 'options' => $options ], 200 );
		}
		catch ( Exception $e ) {
			return new WP_REST_Response([ 'success' => false, 'message' => $e->getMessage() ], 500 );
	...
	public function rest_add( $request )
    {
        try {
            ...
            if ( empty( $name ) || empty( $code ) ) {
                throw new Exception( __( 'Name and code are required.', 'snippet-vault' ) );
            }
	...
	public function rest_update( $request )
    {
        try {
            ...
            $id = $params['id'];
            if ( empty( $id ) ) {
                throw new Exception( __( 'ID is required.', 'snippet-vault' ) );
            }
    ...
    public function rest_lint( $request ) {
        try {
            ...
            if ( empty( $code ) ) {
                throw new Exception( __( 'Code is required.', 'snippet-vault' ) );
            }
