...
class Meow_MWCODE_Modules_Snippet
{
    ...
    public function create_or_update_function_snippet( $params )
    {
        ...
        foreach ( $params['functionArgs'] as $argName ) {
            if ( !empty( $argName ) ) {
                ...
                $this->core->log( '❌ ( Snippet Vault ) Empty argument name.' );
            }
        }
    ...
    public function validate( $params )
    {
        ...
        if ( isset( $params['endpoint'] ) && !empty( $params['endpoint'] ) ) {
            ...
            if ( $exists ) {
                $errors[] = __( 'The endpoint is already used.', 'snippet-vault' );
            }
        }
        ...
        if ( isset( $params['scope'] ) && !empty( $params['scope'] ) ) {
            ...
            if ( !in_array( $params['scope'], $valide_scopes ) ) {
                $errors[] = sprintf( __( 'Invalid scope: %s', 'snippet-vault' ), $params['scope'] );
            }
        }
        ...
        if ( isset( $params['scope'] ) && $params['scope'] === 'function' ) {
            if ( !isset( $params['functionName'] ) || empty( $params['functionName'] ) ) {
                $errors[] = __( 'Function name is required.', 'snippet-vault' );
            }
        }

        if ( isset( $params['functionError'] ) && !empty( $params['functionError'] ) ) {
            $errors[] = __( 'An error was not resolved. (' . $params['functionError'] . ')', 'snippet-vault' );
        }
    ...
    {
        if ( !$this->check_db() ) {
            throw new Exception( __( 'Could not access the database.', 'snippet-vault' ) );
        }
    ...
    public function insert( $insert_data )
    {
        ...
        if ( !$id ) {
            throw new Exception( __( 'Could not insert the snippet.', 'snippet-vault' ) );
        }
        return $id;
    }
    ...
    public function update( $update_data )
    {
        ...
        if ( count( $data ) === 0 ) {
            throw new Exception( __( 'No data to update.', 'snippet-vault' ) );
        }
        $data['updated'] = date( 'Y-m-d H:i:s' );
    ...
    public function force_disable( $id )
    {
        ...
        if ( $result === false ) {
            throw new Exception( __( 'Could not disable the snippet.', 'snippet-vault' ) );
        }
        return $result;
    }
    ...
    public function delete( $delete_data )
    {
        ...
        if ( $result === false ) {
            throw new Exception( __( 'Could not delete the snippet.', 'snippet-vault' ) );
        }
        return $result;
    }
    ...
    public function delete_all(  )
    {
        ...
        if ( $result === false ) {
            throw new Exception( __( 'Could not delete all snippets.', 'snippet-vault' ) );
        }
        return $result;
    }
    ...
    function create_db(  )
    {
        $this->core->log( '💾 ( Snippet Vault ) Creating Table: ' . $this->table_name );
        try {
            ...
            $this->core->log( '💾 ( Snippet Vault ) Create table request: ' . $sql );
            require_once( ABSPATH . 'wp-admin/includes/upgrade.php' );
            dbDelta( $sql );
        } catch ( Exception $e ) {
            $this->core->log( '💾 ( Snippet Vault ) Error creating Table: ' . $e->getMessage(  ) );
        }
    ...
    function check_db(  )
    {
        if ( $this->does_table_exist( $this->table_name ) ) {
            ...
            $this->create_db(  );
            $this->core->log( '💾 ( Snippet Vault ) Table created, checking if it was successful.' );
    ...
    private function check_columns(  )
    {
        ...
        $this->core->log( '💾 ( Snippet Vault ) Database version is ' . $db_version . ', upgrading to ' . $this->mwcode_db_snippet_version . '.' );
    ...
    private function does_table_exist( $table_name )
    {
        ...
        try {
            ...
            $this->core->log( '💾 ( Snippet Vault ) Database Check 1 Error: ' . $e->getMessage(  ) );
        }
        ...
        if ( !$found ) {
            try {
                ...
                $this->core->log( '💾 ( Snippet Vault ) Database Check 2 Error: ' . $e->getMessage(  ) );
            }
        }

        if ( !$found ) {
            $this->core->log( '💾 ( Snippet Vault ) Database table doesn\'t seem to exist.' );
        }
