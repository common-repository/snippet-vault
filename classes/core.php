...
class Meow_MWCODE_Core
{
	...
	function run_snippet( $id, $args = [], $params = [] )
	{
		...
		if ( empty( $params['code'] ) ) {
			throw new Exception( 'Snippet Vault: The snippet code appears to be empty.' );
		}
		
		if ( empty( $params['name'] ) || ! str_contains( $params['code'], $params['name'] ) ) {
			throw new Exception( "Snippet Vault: Function name does not match. The name should be {$params['name']}." );
		}
		...
		try {
			...
			//$this->log('Snippet Vault: Error executing the function: ' . $e->getMessage());
